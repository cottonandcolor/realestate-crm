...
export function LeadsTable({
  ...
  onFocusLeadHandled?: () => void;
  openNotesLeadId?: string | null;
  onOpenNotesHandled?: () => void;
  demoMode?: boolean;
}) {
  ...
  useEffect(() => {
    ...
    if (lead) focusLeadInTable(lead);
    onFocusLeadHandled?.();
  }, [focusLeadId, leads, onFocusLeadHandled]);

  useEffect(() => {
    if (!openNotesLeadId) return;
    const lead = leads.find((l) => l.id === openNotesLeadId);
    if (lead) setActiveLead(lead);
    onOpenNotesHandled?.();
  }, [openNotesLeadId, leads, onOpenNotesHandled]);
